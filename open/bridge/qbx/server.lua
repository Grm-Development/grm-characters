...
local FETCH_USER_CHARACTERS = [[
    SELECT `disabled`, `citizenid`, `cid`, DATE_FORMAT(created, '%d/%m/%Y') AS createdAt, `charinfo` FROM `players` WHERE `license` = ? OR `license` = ? LIMIT ?
]]
...
function bridge.playerLogout(source)
    warn("use qbox logout command!") --exports.qbx_core:Logout(source)
end
...
function bridge.getUserCharacters(source, limit)
    local license_1 = GetPlayerIdentifierByType(source, 'license')  
    local license_2 = GetPlayerIdentifierByType(source, 'license2')
    ...
    return lib.array.map(
        ...
        function(element, index)
            ...
            return {
                ...
                gender = (charinfo.gender == 0) and "m" or "f"
            }
        end
    )
end
